...
import com.ych.hilibrary.extension.edits
import com.ych.hilibrary.extension.sharedPreferences
import com.ych.hilibrary.log.HiLog
import java.util.*

/**
 * TODO:应用程序全局的通用工具类，功能比较单一，经常被复用的功能，应该封装到此工具类当中，从而给全局代码提供方面的操作。
 *
 * @author ych
 * @since 17/2/18
 */
object GlobalUtil {
    ...
    val appPackage: String
        get() = AppGlobals.get()!!.packageName
     ...
     */
    val appName: String
        get() = AppGlobals.get()!!.resources.getString(AppGlobals.get()!!.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = AppGlobals.get()!!.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            AppGlobals.get()!!.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            AppGlobals.get()!!.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(AppGlobals.get()!!.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
                    HiLog.d(TAG, "get android_id with error", e)
                }
            ...
            sharedPreferences.edits { putString("uuid", uuid) }
            deviceSerial = uuid
            return deviceSerial.toString()
        } else {
            return deviceSerial.toString()
    ...
    fun getString(resId: Int): String {
        return AppGlobals.get()!!.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return AppGlobals.get()!!.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return AppGlobals.get()!!.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = AppGlobals.get()!!.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
        } catch (e: PackageManager.NameNotFoundException) {
            HiLog.w(TAG, e.message, e)
        }
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            AppGlobals.get()!!.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = AppGlobals.get()!!.packageManager
